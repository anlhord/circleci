...
func compileHandler(w http.ResponseWriter, r *http.Request) {
	...
	resp, err := compileAndRun(&req, nil)
	if err != nil {
...
func compileAndRun(req *Request, sub *Submission) (*Response, error) {
	...
	if err := ioutil.WriteFile(in, []byte(sub.Data.Body), 0400); err != nil {
	...
	cmd := exec.Command("./gccgo/bin/gccgo", in, "-static-libgo", "-o", exe)
...
func healthCheck() error {
	resp, err := compileAndRun(&Request{Body: healthProg}, nil)
	if err != nil {
		return err
	}
	if resp.Errors != "" {
...
func test() {
	...
	for _, t := range tests {
		resp, err := compileAndRun(&Request{Body: t.prog}, nil)
		if err != nil {
			log.Fatal(err)
		}
		if t.errors != "" {
